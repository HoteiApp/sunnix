...
func ExtractFunctionsPluginsDinamic(pluginName, function string, args ...interface{}) interface{} {
	if _, err := os.Stat(system.Path + "/plugins/" + pluginName + ".dinamic"); os.IsNotExist(err) {
		fmt.Println("Plugins " + pluginName + ".plugins not found")
		return nil
	} else {
		plug, err := plugin.Open(system.Path + "/plugins/" + pluginName + ".dinamic")
		if err != nil {
			fmt.Println(err)
		}
