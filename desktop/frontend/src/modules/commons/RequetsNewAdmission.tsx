...
// -- New Struct
import { FormRequestNewClient, DiagnosticTable, Client, Active } from "../../models";
...
const RequetsNewAdmission = ({ show, relad, closed, client, active }: Props) => {
    ...
    const [requestNewClient, setRequestNewClient] = useState<FormRequestNewClient>({
        client_id: parseInt(client?.id?.toString() || "0") ?? 0,
        referring_agency: "Sunissup",
        referring_person: active?.activeUser?.Record?.fullname ?? "",
        cell_phone: active?.activeUser?.Record?.cell_phone ?? "",
        fax: "",
        email: active?.activeUser?.Record?.email ?? "",
        date: "",
        ...
        legal_guardian: client?.legal_guardian ?? "",
        relationship: client?.relationship ?? "",
        cell_phone_guardian: client?.cell_phone_guardian ?? "",

        medicalid: client?.medicaid ?? "",
        gold_card_number: client?.gold_card_number ?? "",
        ...
        <div className='pt-2'>
            ...
            {isValid === true &&
                <Button
                    ...
                    onClick={() => {
                        // console.log(requestNewClient.client_id);
