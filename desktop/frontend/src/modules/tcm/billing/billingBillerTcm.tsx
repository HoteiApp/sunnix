...
import { Active, TcmsNotesBill, Demografic, ClientsBill, Notes } from "../../../models";
import { useNoteInvoiced, useNotePeriodTcm, useBillApprove, useNotePaid, useNotePaidUnits } from "../../../hooks/modules/tcm";
...
const BillingBillerTcm = ({ active, relad, id, period }: Props) => {
    ...
    // ---------------- Render FINANCE ACTION
    const [showDetails, setShowDetails] = useState(false);
    const [DetailsNote, setDetailsNote] = useState<Notes | undefined>(undefined);

    const isFinanceSignature = (signature: string | undefined) => signature === "data:image/png;base64,";
    const hasPartialPayment = (clients: ClientsBill[] | undefined, paymentType: 'paid2' | 'paid3') =>
        clients?.some(client => client.notes.some(note => note[paymentType] === "Partial"));

    const renderFinishedButton = (billingId: number | undefined, number: string) => (
        <div className="w-auto text-center place-items-center flex p-2 h-full border-l border-r border-primary">
            <div
                ...
                onClick={() => confirmBill('bottom-right', billingId || 0, number)}
            >
