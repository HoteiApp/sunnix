...
// ------
import { Active, Notes, Supervisons, Event, Fortnight } from "../../../models";
// TODO New struct
import { useTCMClientsActive, useDateDiary } from "../../../hooks/modules/tcm";
// -- Component
...
const TCMDashboard = ({ active, relad }: Props) => {
  ...
  const [supervisons, setSupervisons] = useState<Supervisons[]>([]);
  const [events, setEvents] = useState<Event[]>([]);
  const [payments, setPayments] = useState<Fortnight[]>([]);
  ...
  const dateCellRender = (value: Dayjs) => {
            ...
            if (!renderedClients.has(client.id)) {
              ...
              return (
                ...
                <li key={client.id} className='flex'>
                  ...
                  />
                  {(new Date(dates) < new Date(active?.activeUser?.WeekActive?.start ?? "")) ? (
                    <s className='pl-2 text-gray-500'>{client.first_name} {client.last_name}</s>
                  ) : (new Date(dates) > new Date(active?.activeUser?.WeekActive?.end ?? "")) ? (
    ...
    // const renderedReminders = supervisons.map((item) => {
    ...
    // // --- Payments
    // const renderedPayments = payments.map((item) => {
    //   let datesPayments = new Date(item.pay_date);
    //   if (dates === formatDate(datesPayments)) {
    //     numPayments++;
    //     return (
    //       <>
    //         <Tooltip target={`.custom-target-payments-${item.ID}`} />
    //         <li key={item.ID}
    //           className={`custom-target-payments-${item.ID} flex`}
    //           data-pr-tooltip={`Payments from the ${item.start} to the ${item.end_second} `}
    //           data-pr-position="top"
    //         >
    ...
    //             severity={'danger'}
    //             className={classNames(
    //               `custom-target-payments-${item.ID}`,
    //             )}
    //             data-pr-tooltip={`Payment`}
    //             data-pr-position="top"
    //           />
    //           {item.paid ? (
    //             <s className='pl-2 text-gray-400'>Payday</s>
    //           ) : dates > (active?.activeUser?.WeekActive?.end ?? "") ? (
    //             <i className='pl-2'>Payday</i>
    //           ) : (
    //             <span className='pl-2'>Payday</span>
    //           )}
    //         </li>
    //       </>
    //     );
    ...
    const renderedReminders = allItems.map((item) => {
      if (dates === formatDate(item.date)) {
        if (item.type === 'supervision') numReminders++;
        if (item.type === 'payment') numPayments++;

        const isSupervision = item.type === 'supervision';
        const tooltipText = isSupervision
          ? item.title
          : `Payments from the ${item.start} to the ${item.end_second}`;

        const badgeValue = isSupervision ? 'S' : 'P';
        const badgeSeverity = isSupervision ? 'success' : 'danger';
        const itemClass = `custom-target-${item.type}-${item.id}`;

        const textContent = isSupervision
          ? item.title.split(" ").slice(0, 2).join(" ") + "..."
          : 'Payday';

        const conditionalText = item.completed
          ? <s className='pl-2 text-gray-400'>{textContent}</s>
          : dates > (active?.activeUser?.WeekActive?.end ?? "")
            ? <i className='pl-2'>{textContent}</i>
            : <span className='pl-2'>{textContent}</span>;

        return (
          <>
            <Tooltip target={`.${itemClass}`} />
            <li key={item.id}
            ...
            >
              <Badge value={badgeValue}
                ...
                data-pr-tooltip={isSupervision ? 'Supervision' : 'Payment'}
                data-pr-position="top"
              />
              {conditionalText}
            </li>
    ...
    // const safeRenderedPayments = (renderedPayments || []).flat().filter((item): item is JSX.Element => item !== null && item !== undefined);
        ...
        <div className={classNames(
          ...
          dates === dateSelect ? 'bg-green-100' : (
            ((dayjs(dates).isSame(dayjs(active?.activeUser?.WeekActive?.start ?? "")) || dayjs(dates).isAfter(dayjs(active?.activeUser?.WeekActive?.start ?? ""))) &&
              ...
              'bg-blue-100' : (numNotes > 0 || numEvents > 0 || numReminders > 0) ?
          ...
          <div className={classNames(
            ...
            <div className='text-center'>
              {
                (new Date(dates) > new Date(active?.activeUser?.WeekActive?.start ?? "")) &&
                (numEvents || numNotes || numReminders) > 0 &&
            ...
            <div className='w-1/2 text-right justify-content-center'>
              {numReminders > 0 &&
                current <= dates &&
                new Date(dates).getMonth() === new Date(current).getMonth() &&
                new Date(dates) <= new Date(new Date(current).setDate(new Date(current).getDate() + 7)) && (
                  <>
  ...
  return (
    ...
    <div className="relative w-full h-screen">
          ...
          return (
            <div className='w-full flex'>
              ...
              <div className='w-1/2 text-right pr-5 pt-5 items-center'>
                ...
                <Badge value="P" className='ml-3' severity={'danger'} />Payments
      ...
      <Dialog header={dateSelect} maximizable visible={visible} modal={true} style={{ width: '60vw' }} onHide={() => setVisible(false)}>
        <DiaryTcm date={dateSelect} active={active} tcmNotes={notes} myClients={myClientsActive} supervisions={supervisons} payments={payments} relad={relad} />
      </Dialog>
    </div>
  );
};
